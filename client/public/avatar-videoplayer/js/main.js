...
textForConnectionId.value = 'Tank001';//getRandom();
...
function showInfo(res) {
  ...
  infoDiv.innerHTML += `<h4>Info</h4> useWebSocket : ${res.useWebSocket} ==> this.signaling = new WebSocketSignaling();`;
}
...
async function setupVideoPlayer(elements) {
  ...
  await videoPlayer.setupConnection(useWebSocket, connectionId);
